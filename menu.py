...
while run:
    ...
    if menu_state == "main":
        ...
        if play_button.draw(screen):
            ...
            # Step 1: Dual Character Selection (P1 & P2 together)
            print("\n=== CHARACTER SELECTION ===")
            ...
            if selected_chars:
                ...
                print(f"✓ Player 1 selected: {selected_char_p1}")
                print(f"✓ Player 2 selected: {selected_char_p2}")
                ...
                if selected_arena:
                    print(f"\n{'='*60}")
                    print(f"🎮 GAME START!")
                    print(f"   Player 1: {selected_char_p1}")
                    print(f"   Player 2: {selected_char_p2}")
                    print(f"   Arena: {selected_arena}")
                    print(f"{'='*60}\n")
                    
                    # Di sini Anda bisa memulai game dengan 2 karakter dan arena yang dipilih
                    # Untuk sekarang, kita kembali ke menu
                    pass
                else:
                    # User membatalkan arena selection
                    print("Arena selection cancelled, returning to menu")
            else:
                # User membatalkan character selection
                print("Character selection cancelled, returning to menu")
            
            # Reinitialize screen untuk kembali ke menu
            menu_state = "main"
            screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
            pygame.display.set_caption("Game Menu")
