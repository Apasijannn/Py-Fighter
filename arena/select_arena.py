...
class ArenaSlot:
    def __init__(self, arena_data, x, y, slot_width=300, slot_height=200):
        ...
        try:
            ...
            full_path = os.path.abspath(arena_data["path"])
            print(f"\n[DEBUG] Loading {self.name}:")
            print(f"  - Path: {arena_data['path']}")
            print(f"  - Full path: {full_path}")
            print(f"  - Exists: {os.path.exists(arena_data['path'])}")
            
            # Load and scale arena image
            ...
            target_height = slot_height - 60  # Leave space for name
...
class ArenaSelection:
    ...
    def confirm_selection(self):
        if self.selected_index is not None:
            ...
            print(f"✓ Arena Confirmed: {selected.name}")
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if arena:
                            print(f"Starting battle in {arena}!")
